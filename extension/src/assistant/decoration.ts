...
export default class Decoration {
    ...
    show(prediction: Prediction) {
        ...
        predictedList.forEach((pred: utils.StringDifference) => {
            const decoration = vscode.window.createTextEditorDecorationType({after: {
                contentText: pred.text,
                color: new vscode.ThemeColor("editorGhostText.foreground"),
            }});
            this._decorations.push(decoration);

            const posStart = new vscode.Position(prediction.position.line, pred.index);
            const posEnd = new vscode.Position(prediction.position.line, pred.index + pred.index + pred.text.length);
